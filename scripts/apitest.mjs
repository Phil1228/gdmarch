...
// server.mjs exports default = handleRequest (Vercel style)
const mod = await import('../server.mjs');
const handleRequest = mod.default || mod.handleRequest;
console.log('has default export:', typeof handleRequest === 'function');

function makeReq(url, method = 'GET') { return { url, method, headers: { 'x-vercel-original-url': url }, on() {} }; }
function makeRes() {
  const self = { headersSent: false, statusCode: 200, body: '', writeHead(c){this.statusCode=c;}, end(b){this.body=b;this._resolve(b);}, on(){} };
  self._promise = new Promise((r)=>(self._resolve=r)); return self;
}
async function call(url, method='GET', body=null) {
  ...
  await handleRequest(req, res); return { status: res.statusCode, body: res.body };
}
const eid = await db.createEvent('API測試', { scoring_mode:'team', rounds:3, round_rule:'rounds' });
const page = await call(`/r/${eid}`);
console.log('GET /r/<id>:', page.status, 'isHtml:', page.body.includes('掼'));
...
console.log('404:', nf.status);
try { unlinkSync(TEST_DB); } catch {}
console.log('\n✅ VERCEL-STYLE HANDLER VERIFIED'); process.exit(0);
