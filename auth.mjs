...
export async function register(username, password, displayName = '') {
  ...
  const ex = (await db.execute({ sql: 'SELECT id FROM users WHERE username = ?', args: [username] })).rows[0];
  if (ex) throw new Error('帳號已存在');
  const cnt = (await db.execute('SELECT COUNT(*) AS c FROM users')).rows[0].c;
...
export async function login(username, password) {
  const u = (await db.execute({ sql: 'SELECT * FROM users WHERE username = ?', args: [username] })).rows[0];
  ...
  await db.execute({ sql: 'INSERT INTO sessions (token, user_id, expires_at) VALUES (?, ?, ?)', args: [token, u.id, expires] });
...
export async function getUserByToken(token) {
  ...
  const s = (await db.execute({ sql: 'SELECT * FROM sessions WHERE token = ?', args: [token] })).rows[0];
  if (!s) return null;
  if (s.expires_at && new Date(s.expires_at) < new Date()) {
    await db.execute({ sql: 'DELETE FROM sessions WHERE token = ?', args: [token] });
    return null;
  }
  const u = (await db.execute({ sql: 'SELECT * FROM users WHERE id = ?', args: [s.user_id] })).rows[0];
...
export async function resetPassword(userId, newPassword) {
  ...
  await db.execute({ sql: 'UPDATE users SET password_hash = ? WHERE id = ?', args: [hashPassword(newPassword), userId] });
}

export async function listUsers() {
  return (await db.execute('SELECT id, username, display_name, role, created_at FROM users ORDER BY id')).rows;
}
